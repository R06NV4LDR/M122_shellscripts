...
# Log-Pfad
log_file="/home/rooney/Docs/logs/finance/exchange_${from_currency}${to_currency}.log"
...
if [ $? -ne 0 ]; then
  ...
  echo "$(date) - Fehler bei der Anfrage an die API" >> $log_file
  exit 1
fi
...
if [ -z "$rate" ]; then
  ...
  echo "$(date) - Fehler beim Extrahieren des Wechselkurses" >> $log_file
  exit 1
fi
